...
- 一个Angular单运行时共享依赖的解决方案
- 只启动一个Angular平台(`PlatformRef`),其他子项目进行导出(类es6 module),通过资源文件清单(或单独js文件)进行请求加载
- 支持懒加载路由的子项目请求及其他动态模块的请求
- 支持子项目在开发时代码变更的刷新
- 支持子项目及主项目的splitChunks(包括懒加载等...)
- 支持子项目调用主项目导出的命名(export * from 'xxx')
- 支持`可信`的`远程`子项目开发
- 不需要了解各种概念,因为调用处理完全遵循`动态import`的类处理方案
- 通过在编译时构成构成关系,对源代码无侵入性,不需要过多改造
> 只需要调用一个全局的加载函数,返回`Promise<T>`,`T`为子模块定义时的导出类型,返回后即可正常使用
- 子模块构建速度极快,且在大多数时间不需要向node申请内存
> 当拆为多个子项目时,本身单次构建也也没有太多文件
- 理论上子项目同时可以加载子项目,也就是可以存在多层级子项目的加载
> 简单的理解就是`a`为子项目,同时`a/b`也是子项目,同时在`a`中被调用,以此类推
...
- 主项目通过启动子项目的资源文件/资源配置,来加载子项目资源,并且在加载后实现自动获得子项目的导出,作为懒加载路由,懒加载模块等动态使用的地方调用
...
- 主项目资源清单变化时(大多是增加/减少依赖),需要同步子项目资源清单保持一致
